...
def get_IO_target_bug(target, tool):
    csv_path = os.path.join(BENCHMARK_DIR, "assets", "B-IO-cve.csv")
    f = open(csv_path, "r")
    for line in f:
        line = line.strip()
        if line != "":
            if target == line.split(',')[0]:
                f.close()
                if tool == "smartian" or tool == "IConFuzz":
...
def get_ELSC_target_bug(target, tool):
    ...
    with open(csv_path, "r") as f:
        for line in f:
            ...
            prefix = '-b ' if tool == "smartian" or tool == "IConFuzz" else 'leak kill -target_bug "'
...
def run_fuzzing(benchmark, target, tool, timelimit, opt, cpu_idx):
    ...
    if benchmark == "B-IO" and tool in ["smartian", "IConFuzz", "SmarTest"]:
        opt = opt + " " + str(get_IO_target_bug(targ, tool))
    elif benchmark == "B-ELSC" and tool in ["smartian", "IConFuzz", "SmarTest"]:
